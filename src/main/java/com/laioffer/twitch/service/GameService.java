...
public class GameService {
    ...
    private static final String TOKEN = "Bearer op811oth6mivxruxdscstpk9k2zt4h";
    private static final String CLIENT_ID = "c8fkgudu1mwqu707t185uugv4enjih";
    ...
    private String buildGameURL(String url, String gameName, int limit) {
        if (gameName.equals("")) {
            ...
            try {
                // Encode special characters in URL, e.g. Rick Sun -> Rick%20Sun, no space between inputs!
    ...
    private List<Item> searchStreams(String gameId, int limit) throws TwitchException {
        ...
        for (Item item : streams) {
            ...
            item.setUrl(TWITCH_BASE_URL + item.getBroadcasterName());// only steam doesn't return url, need manually add
        }
        return streams;
    }
    ...
    public Map<String, List<Item>> searchItems(String gameId) throws TwitchException {
        ...
        for (ItemType type : ItemType.values()) {
            itemMap.put(type.toString(), searchByType(gameId, type, DEFAULT_SEARCH_LIMIT)); // based on dif type return dif methods
        }
        return itemMap;
    }
